...
function DNAStrands({ strandCount = 3 }) {
    const rungsPerStrand = 40; // Optimized from 60
    ...
    useFrame((state, delta) => {
        ...
        strands.forEach((strand, sIdx) => {
            for (let i = 0; i < rungsPerStrand; i++) {
                ...
                // Strand 1 - rotating and moving upward
                ...
                // Strand 2 - opposite side, rotating and moving upward
...
function LargeSmoke() {
    const count = 12; // Optimized from 20
    ...
    useFrame((state, delta) => {
        ...
        particles.forEach((p, i) => {
            ...
            // Horizontal drift for natural smoke movement
            ...
            const progress = (p.y + 60) / 120;
            const horizontalScale = p.baseScale * (1 + progress * 2);
            const verticalScale = p.baseScale * (1 + progress * 3);
...
function Scene() {
    return (
        <>
            ...
            <pointLight position={[0, -20, 10]} intensity={8} color="#db2777" distance={120} />

            <Suspense fallback={null}>
                <LargeSmoke />
                <DNAStrands strandCount={3} />
            </Suspense>
        </>
    );
}
